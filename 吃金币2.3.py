...
def draw():
    ...
    if g_staus == STATUS_RUNNING:
        g_actor.draw()
        g_stuff.draw()
        screen.draw.text(f"剩余时间: {g_remain_time} 秒", (10, 10), fontname='noto')
        screen.draw.text(f"现有分数: {g_score}", (10, 50), fontname='noto')
    elif g_staus == STATUS_PAUSE:
        screen.draw.text(f"游戏已暂停", (500, 50), fontname='noto')
    elif g_staus == STATUS_FINISH:
        screen.draw.text(f"游戏结束，得分: {g_score}", (500, 50), fontname='noto')
